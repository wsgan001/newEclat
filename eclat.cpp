...
void parse_args(int argc, char **argv)
{
   ...
   if (argc < 2){
      ...
      cout << "\t-x (no offset?) (default=false)\n";
      exit(-1);
   }
   else{
     while ((c=getopt(argc,argv,"a:bc:d:i:lm:op:s:S:xz:"))!=-1){
       switch(c){
       case 'a':
...
//    cout << "ORDER :\n";
   for (i=0; i < DCB->NumF1; ++i){
      ...
      //cout << i << " " << DCB->ParentClass[i]->val << " " << DCB->FreqIdx[i] << endl;
   }
   //cout << endl;
